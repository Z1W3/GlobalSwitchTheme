package cattt.gst.library.base.helper;
...
import cattt.gst.library.base.enums.DrawableStates;
...
public class ParseAssetsHelper {
     ...
     * 该参数的值均为bool类型
     */
    private static final String[] SELECTOR_PARAM_NAMES = {
            "state_checked",
            "state_pressed",
            "state_selected",
            "state_checkable",
     ...
     * 解析colors.xml中的内容
     */
    public Vector<ColorsBean> parseColorsXml(String target) throws IOException {
        if (TextUtils.isEmpty(target)) {
            throw new NullPointerException("Can't be null.");
        }
        return this.parseColorsXml(new File(target));
    }
     ...
     * 解析colors.xml中的内容
     */
    public Vector<ColorsBean> parseColorsXml(FileInputStream inputStream) throws IOException {
        if (inputStream == null) {
            throw new NullPointerException("Can't be null.");
        }
     ...
     * 解析selectors.xml中的内容
     */
    public Vector<SelectorsBean> parseSelectorsXml(String target) throws IOException {
        if (TextUtils.isEmpty(target)) {
            throw new NullPointerException("Can't be null.");
        }
        return this.parseSelectorsXml(new File(target));
    }
     ...
     * 解析selectors.xml中的内容
     */
    public Vector<SelectorsBean> parseSelectorsXml(FileInputStream inputStream, String path) throws IOException {
     ...
     */
    public Vector<BackgroundBean> parseBackgroundsXml(String target) throws IOException {
        if (TextUtils.isEmpty(target)) {
            throw new NullPointerException("Can't be null.");
        }
        return this.parseBackgroundsXml(new File(target));
    }

    /**
     * 解析backgrounds.xml中的内容
     */
    public Vector<BackgroundBean> parseBackgroundsXml(File target) throws IOException {
        if (target == null) {
            throw new NullPointerException("Can't be null.");
        }
     ...
     * 解析backgrounds.xml中的内容
     */
    public Vector<BackgroundBean> parseBackgroundsXml(FileInputStream inputStream, String path) throws IOException {
        if (inputStream == null || TextUtils.isEmpty(path)) {
            throw new NullPointerException("Can't be null.");
        }
        final Vector<BackgroundBean> beans = new Vector<>();
        final String rootPath = path.substring(0, path.lastIndexOf("/"));
        try {
            final XmlPullParser xml = getXmlPullParser(inputStream);
            int eventType = xml.getEventType();
    ...
    private XmlPullParser getXmlPullParser(final FileInputStream inputStream) throws XmlPullParserException {
    ...
    public static ParseAssetsHelper get() {
